...
class Semana extends CActiveRecord
{
	...
	public function afterValidate()
	{
		...
		{
			if (!preg_match("/^2016-05-(1[6-9]|2[1-2]) (0[6-9]|1[0-9]|2[0-2]):[[:digit:]][[:digit:]]$/",$this->fecha_ini))
			{
				$this->addError('fecha_ini', 'La fecha de inicio es incorrecta, por favor selecciona la fecha en el calendario que se despliega.');
				$valido = false;
			}

			if (!preg_match("/^2016-05-(1[6-9]|2[1-2]) (0[6-9]|1[0-9]|2[0-2]):[[:digit:]][[:digit:]]$/",$this->fecha_fin))
			{
